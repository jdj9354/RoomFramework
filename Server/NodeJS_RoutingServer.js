...
var SocketIORoutingServer = net.createServer(function(conn){
	...
	conn.on('end', function() {
		console.log("stop!!!");
		for(var i=0; i<newSocketIOServer.length; i++){
			if(conn.remoteAddress == newSocketIOServer[i].address)
				RDWrapperChildServers.splice(i,1);
		}
		//Need to implement removing room info
	});
	...
	conn.on('data',function(data){
		...
		for(var i=0; i<dataArray.length; i++){
			...
			case "0":
				...
				if(!roomInfoHashMap.has(roomName)){
					...
					for(var j=0; j<RDWrapperChildServers.length; j++){
						if(RDWrapperChildServers[j].address == serverAddr
			...
			case "1":
				...
				var serverAddr = serverInfo.addr;
				var serverPort = serverInfo.port;
				
				roomInfoHashMap.remove(roomName);
				
				for(var j=0; j < RDWrapperChildServers.length; j++){
					if(RDWrapperChildServers[j].address == serverAddr && RDWrapperChildServers[j].port == serverPort){
...
RoutingServerIo.sockets.on('connection', function (socket){
	...
	socket.on('AddrPortReq',function(data){
		...
		if(!roomInfoHashMap.has(data)){
			...
			var retObj = {
							addr : RDWrapperChildServers[minIndex].address,
			...
			case "AddrPortReq" : 
				...
				if(!roomInfoHashMap.has(roomName)){
					...
					var retString = "AddrPortRet" + " " +
									RDWrapperChildServers[minIndex].address + " " +
